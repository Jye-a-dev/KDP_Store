  ...
  const fetchAdminDashboardData = useCallback(async () => {
    ...
    try {
      ...
      const [ordersRes, orderStatsRes, userStatsRes] = await Promise.all([
        fetchWithTimeout(`${API_BASE_URL}/orders?limit=5&sort_by=created_at&sort_order=DESC`, { headers }),
        fetchWithTimeout(`${API_BASE_URL}/orders/count`, { headers }),
        fetchWithTimeout(`${API_BASE_URL}/users/count`, { headers }),
      ]);
      ...
      const ordersData = (await ordersRes.json()) as { data: Order[] } | Order[];
