...
export function useOrders() {
    ...
    async (userId: string, params: OrderQueryParams = {}) => {
      ...
      try {
        ...
        const [ordersRes, statsRes] = await Promise.all([
          fetchWithTimeout(
            `${API_BASE_URL}/orders?${query.toString()}`,
            { headers }
          ),
          fetchWithTimeout(`${API_BASE_URL}/orders/count?user_id=${userId}`, { headers }),
        ]);
  ...
  const fetchAdminOrders = useCallback(async () => {
    ...
    try {
      const res = await fetchWithTimeout(
        `${API_BASE_URL}/orders?limit=5&sort_by=created_at&sort_order=DESC`,
        { headers: getHeaders() }
      );
  ...
  const fetchOrdersList = useCallback(
    async (params: OrderQueryParams = {}) => {
      ...
      try {
        ...
        const res = await fetchWithTimeout(`${API_BASE_URL}/orders?${query.toString()}`, {
          headers: getHeaders(),
        });
  ...
  const fetchOrderStats = useCallback(async (params: OrderQueryParams = {}) => {
    try {
      ...
      const res = await fetchWithTimeout(`${API_BASE_URL}/orders/count?${query.toString()}`, { headers: getHeaders() });
